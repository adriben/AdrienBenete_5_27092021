...
async function displayCamera() {
  //affichage de la camera selectionner dans la page precedente
  ...
  const camera = await getCameras(cameraID)
    ...
    .catch((err) => {
      return (document.querySelector(".productsDisplay").innerHTML =
        "<h3 class='text-center mt-5'>Il semble que le serveur ne soit pas actif, veuillez reessayez dans 1 minute <br/>Nous sommes désolé pour la gêne occasionnée...</h3>");
    });
...
addToCartButton.addEventListener("click", (event) => {
  ...
  document.querySelector(".alert").innerText =
    "Vous avez bien ajouté cette camera à votre panier, merci";
...
function addToCart() {
  ...
  let cameraAdded = {
    //on creer un objet avec des valeurs
  ...
  if (cameraLocalStorage) {
    ...
    for (let i = 0; i < cameraLocalStorage.length; i++) {
      if (
        cameraLocalStorage[i].id == cameraAdded.id &&
        cameraLocalStorage[i].lens == cameraAdded.lens
      ) {
        cameraLocalStorage[i].price = parseInt(cameraLocalStorage[i].price);
        cameraLocalStorage[i].quantity =
          parseInt(cameraLocalStorage[i].quantity) +
          parseInt(cameraAdded.quantity);
        localStorage.setItem(
          "productInCart",
          JSON.stringify(cameraLocalStorage)
        );
        break;
    ...
    //si il n y a rien dans le localstorgae on creer un tableau vide et on ajoute l objet
    cameraLocalStorage = [];
    cameraLocalStorage.push(cameraAdded);
    cameraAdded.quantity = parseInt(cameraAdded.quantity);
    cameraAdded.price = parseInt(cameraAdded.price);
